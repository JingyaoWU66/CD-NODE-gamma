...
class AdaptiveStepsizeODESolver(object):
    ...
    def integrate(self, t):
        ...
        ##############################################
        self.before_integrate(t) ## from where y0 is changed
        ...
        for i in range(1, len(t)):
            ...
            if len(self.y0)==1:
                if i==1:
                    fea_next = self.xa[-1:, :, :]  # taking next features
                    self.xa = torch.cat((self.xa, fea_next), 0)
                    del fea_next
                y = self.advance(t[i])
                #print(y)
                #print(t[i])
                ...
                y = self.advance(t[i])
                #print(y) #this is where the nan comes from
            ##############################################
            solution.append(y)
