...
def _runge_kutta_step(func, y0, x0, f0, t0, dt, tableau):
    ...
    for alpha_i, beta_i in zip(tableau.alpha, tableau.beta):
        ...
        if len(y0)==1:
            ...
            y_new = (torch.cat((yi[0], x0), 1),)
            tuple(k_.append(f_) for k_, f_ in zip(k, func(ti, y_new))) ## add y0 Jennifer
        else:
            yi = tuple(y0_ + _scaled_dot_product(dt, beta_i, k_) for y0_, k_ in zip(y0, k))
            yi_new = tuple([torch.cat((yi[0], x0), 1)] + [yi[1]] + [yi[2]] + [yi[3]])
            #print(yi_new)
            c_flag = 1
            for k_, f_ in zip(k, func(ti, yi_new)):
                if c_flag == 2:
                    k_.append(f_[:, :1])
                    c_flag = c_flag + 1
                else:
